class QuestsController < ApplicationController
  ...
  # GET /quests.json
  def index
    @quests = Quest.all(:order => "completed ASC, level DESC")
  ...
  def failure
    ...
    unless @quest.completed
      ...
      @quest.succeded = false
    end
    respond_to do |format|
      if @quest.save
        format.html { redirect_to @quest, notice: 'Quest failed!' }
  ...
  def success
    ...
    unless @quest.completed
      @quest.completed = true
      @quest.succeded = true
      Hero.assign_experience @quest.experience
    end
    respond_to do |format|
      if @quest.save
